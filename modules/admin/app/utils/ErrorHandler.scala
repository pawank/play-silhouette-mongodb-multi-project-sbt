...
class ErrorHandler @Inject() (
  ...
  override def onNotFound(request: RequestHeader, message: String): Future[Result] = Future.successful {
    NotFound(Json.obj("message" -> "Resource not found"))
  }
  ...
  override def onNotAuthenticated(request: RequestHeader, messages: Messages): Option[Future[Result]] = {
    Some(Future.successful(Ok(Json.obj("message" -> Messages("not.authenticated")(messages)))))
  }
  ...
  override def onNotAuthorized(request: RequestHeader, messages: Messages): Option[Future[Result]] = {
    Some(Future.successful(Unauthorized(Json.obj("message" -> Messages("access.denied")(messages)))))
  }
}
