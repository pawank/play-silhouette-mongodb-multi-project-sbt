...
import utils.{ MailService, Mailer, Utils }
...
class SignUpController @Inject() (
  ...
  def signUpRequest = UserAwareAction.async { implicit request =>
    request.identity match {
      ...
      case None => Future.successful(Ok(views.html.web.signUp(SignUpForm.formSignUp)))
  ...
  def signUpRequestRegistration = Action.async { implicit request =>
    SignUpForm.formSignUp.bindFromRequest.fold(
      form => Future.successful(BadRequest(views.html.web.signUp(form))),
      signUpData => {
        ...
        userService.retrieve(loginInfo).flatMap {
          ...
          case None =>
            ...
            val token = TokenUser(UUID.randomUUID().toString, signUpData.company, signUpData.email, (new DateTime()).plusHours(TokenUser.hoursTillExpiry), true, signUpData.firstName, signUpData.lastName)
            tokenService.create(token)
            Mailer.welcome(signUpData, link = routes.SignUpController.signUpCompletion(token.id).absoluteURL())(mailService)
            Future.successful(Ok(views.html.web.auth.almostSignedUp(signUpData)))
      ...
      authInfoRepository.find(loginInfo)(ClassTag(classOf[PasswordInfo])).flatMap {
        case Some(authInfo) =>
          val user = User(
            ...
            loginInfo = loginInfo,
            company = Some(tokenUser.company),
  ...
  private def executeForToken(token: String, isSignUp: Boolean, f: TokenUser => Future[Result]): Future[Result] = {
    tokenService.retrieve(token).flatMap[Result] { optTokenUser =>
      optTokenUser match {
        ...
        case _ => Future.successful(NotFound(views.html.web.auth.invalidToken()))
      }
    }
  }
}
